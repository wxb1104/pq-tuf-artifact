...
fn main() {
    ...
    // PQ-TUF portable tier: setting PQTUF_NO_AVX2 skips the 4x AVX2 Keccak
    // (SHA-3) helper entirely. Clean-C algorithm cores then link against the
    // scalar fips202.c in pqclean_common, yielding a true AVX2-free binary.
    // Unset (default) keeps upstream behaviour for the AVX2-optimized tier.
    let no_avx2 = env::var("PQTUF_NO_AVX2").is_ok();

    if !no_avx2 && (target_arch == "x86" || target_arch == "x86_64") {
        if target_env == "msvc" {
